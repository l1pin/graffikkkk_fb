function getDataBySql(strSQL = "SELECT * FROM `ads_collection` WHERE `source` = 'facebook'") {
    try {
    ...
    `;
        ...
        allRows.forEach(row => {
            ...
            if (hasMetrics && (campaignName || groupId)) {
                console.log(`📘 Processing Facebook metrics for buyer: ${buyerInfo.buyer}, group: ${groupName}, date: ${dateStr}`);
        ...
        function processSegment(segmentName, resultMapBySegment, fbDataMapBySegment, segmentType) {
            ...
            for (let i = 0; i < segmentDates.length; i++) {
                ...
                const fbDataSegment = (dayLeads > 0 || daySpend > 0) ? (fbDataMapBySegment[segmentName] && fbDataMapBySegment[segmentName][dateKey] || createFacebookMetricsObject()) : createFacebookMetricsObject();
